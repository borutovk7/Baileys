/**
 * Eduh Developer & Lia 09-04-26 [WIP]
...
// Eduh Developer & Lia 09-04-26 --- Inject buffer into unifiedResponse.data to support proper rendering of rich messages (ex: tables and code blocks)
...
export const prepareRichResponseMessage = (content) => {
        ...
        if (links) {
            links.forEach((linkField, index) => {
                ...
                const sources = linkField.sources?.map((sourceField) => ({
                    ...
                    source_subtitle: sourceField.subtitle || 'Boruto ',
                ...
                submessages.push({
                    ...
                    inlineEntities: [{
                            ...
                            metadata: {
                                ...
                                reference_url: url,
                                reference_title: linkField.title || 'Acesse nossa rest Api',
                                reference_display_name: linkField.displayName || 'Boruto ',
                                sources: sources || [],
    ...
    const richResponseMessage = proto.AIRichResponseMessage.create({
        ...
        unifiedResponse: {
            data: Buffer.from(JSON.stringify(unified)) // Eduh Developer & Lia 25-04-26 --- Expects "ArrayBufferLike"
        },
        contextInfo: {
            isForwarded: true,
            forwardingScore: 1,
    ...
    // Eduh Developer & Lia 13-05-26 --- Add disclaimer text on richResponseMessage
    if (disclaimerText) {
        botMetadata.messageDisclaimerText = disclaimerText;
    }
    // Eduh Developer & Lia 15-05-26 --- Add responseId from unified directly to botMetadata
    botMetadata.botResponseId = uuid;
    return message;
};
// Eduh Developer & Lia 17-04-26 --- signature and certificateChain for proofs[] field
...
export const wrapToBotForwardedMessage = (richResponseMessage) => ({
    messageContextInfo: {
        botMetadata: {
            // Eduh Developer & Lia 09-04-26 --- TODO: Fill verificationMetadata field
