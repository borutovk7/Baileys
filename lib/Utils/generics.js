...
// Cache interno com vida útil de 12 horas
let cachedVersion = null;
let lastFetchTime = 0;
const VERSION_CACHE_TTL = 12 * 60 * 60 * 1000; // 12 horas
 ...
 * 
 * Tem vida útil de 12 horas (cache).
 * Só busca nova versão quando:
 *   - O cache expirou (12h), ou
 *   - Forçado via options.force = true, ou
 *   - A conexão cair com erro de versão (use shouldRefreshVersion)
 * 
 * Recomendação: Use apenas quando necessário para evitar trocas constantes de versão.
 */
export const fetchLatestBaileysVersion = async (options = {}) => {
    const now = Date.now();
    const forceRefresh = options.force === true;

    // Retorna versão em cache se ainda válida
    if (!forceRefresh && cachedVersion && (now - lastFetchTime) < VERSION_CACHE_TTL) {
        return {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Boom(`Failed to fetch WhatsApp version: ${response.statusText}`, { statusCode: response.status });
        }
        ...
        const version = [2, 3000, +clientRevision];
        ...
        // Se der erro, retorna a versão em cache (se existir) ou a padrão
        if (cachedVersion) {
            return {
                version: cachedVersion,
                isLatest: true,
...
/**
 * Verifica se o erro indica que a versão do WhatsApp está desatualizada/inválida.
 * Use isso para decidir se deve chamar fetchLatestBaileysVersion().
 */
export const shouldRefreshVersion = (error) => {
    if (!error) return false;

    const statusCode = error?.output?.statusCode || error?.statusCode || error?.code;
    const message = error?.message || '';

    const badVersionCodes = [401, 403, 515];
    const badVersionMessages = ['restart required', 'stream error', 'unauthorized', 'forbidden'];

    if (badVersionCodes.includes(statusCode)) return true;

    const lowerMessage = message.toLowerCase();
    return badVersionMessages.some(msg => lowerMessage.includes(msg));
};

/**
 * Função recomendada para obter a versão do WhatsApp.
 * 
 * - Usa cache de 12 horas
 * - Só atualiza quando o cache expira ou quando forçado
 * - Ideal para manter estabilidade
 */
export const getWhatsAppVersion = async (options = {}) => {
    return fetchLatestBaileysVersion(options);
};
